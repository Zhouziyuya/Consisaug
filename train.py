# 加了上下左右翻转的consistency loss的训练文件
...
from utils.loss import ComputeLoss, ConsistencyLossLR, ConsistencyLossUD, ConsistencyLossLRUD, ConsistencyLossNo
...
def step_decay(step, end): # consistency loss权重
    # lr = conf.lr
    # 
    ...
    # lr = 1 * np.minimum(1., 4*step / end) # small to large

    progress = step / float(end)
    lr = 1-progress # large to small
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    consistency_loss_lr = ConsistencyLossLR(model)
    consistency_loss_ud = ConsistencyLossUD(model)
    consistency_loss_lrud = ConsistencyLossLRUD(model)
    consistency_loss_no = ConsistencyLossNo(model)
    kldivloss = nn.KLDivLoss(log_target=True, reduction='batchmean').to(device)
    mseloss = nn.MSELoss().to(device)
    # momentum parameter is increased to 1. during training with a cosine schedule
    momentum_schedule = cosine_scheduler(0.99, 0.9999, epochs, len(train_loader), 5) # warmup_epochs = 5
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, imgs_fliplr, imgs_flipud, img_fliplrud, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            imgs_fliplrud = img_fliplrud.to(device, non_blocking=True).float() / 255
            ...
            if ni <= nw:
                ...
                for j, x in enumerate(optimizer.param_groups):
                    ...
                    x['lr'] = np.interp(ni, xi, [hyp['warmup_bias_lr'] if j == 0 else 0.0, x['initial_lr'] * lf(epoch)]) # 线性插值，x:[0,warmup iter], y:[0,x['initial_lr'] * lf(epoch)]
                    if 'momentum' in x:
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                if opt.consis_mode == 'lr':
                    ...
                    loss_consis, loss_consis_items= consistency_loss_lr(pred, pred_flip, targets.to(device))
                elif opt.consis_mode == 'ud':
                    pred_flip = teacher(imgs_flipud)
                    loss_consis, loss_consis_items= consistency_loss_ud(pred, pred_flip, targets.to(device))
                elif opt.consis_mode == 'lrud':
                    r = random.random()
                    if r < 0.25:
                        pred_flip_t = teacher(imgs_fliplr) # {[16,3,80,80,6], [16,3,40,40,6], [16,3,20,20,6]}
                        loss_consis1, loss_consis_items1= consistency_loss_lr(pred, pred_flip_t, targets.to(device))

                        pred_flip_s = model(imgs_fliplr)
                        loss_consis2, loss_consis_items2= consistency_loss_lr(pred_t, pred_flip_s, targets.to(device))
                    elif r>=0.25 and r<0.5:
                        pred_flip_t = teacher(imgs_flipud)
                        loss_consis1, loss_consis_items1= consistency_loss_ud(pred, pred_flip_t, targets.to(device))

                        pred_flip_s = model(imgs_flipud)
                        loss_consis2, loss_consis_items2= consistency_loss_ud(pred_t, pred_flip_s, targets.to(device))
                    elif r>=0.5 and r<0.75:
                        pred_flip_t = teacher(imgs_fliplrud)
                        loss_consis1, loss_consis_items1= consistency_loss_lrud(pred, pred_flip_t, targets.to(device))

                        pred_flip_s = model(imgs_fliplrud)
                        loss_consis2, loss_consis_items2= consistency_loss_lrud(pred_t, pred_flip_s, targets.to(device))
                    else:
                        # pred_flip = teacher(imgs)
                        # loss_consis, loss_consis_items= consistency_loss_lrud(pred, pred_flip, targets.to(device))
                        loss_consis1, loss_consis_items1=0,(0,0,0)
                        loss_consis2, loss_consis_items2=0,(0,0,0)
                ...
                # loss = step_decay(epoch, epochs)*loss_consis+loss_sup
            ...
            if ni - last_opt_step >= accumulate: # ni当前的iteration
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                # strip_optimizer(f)  # strip optimizers 把model模型替换成ema模型
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
...
def parse_opt(known=False):
    ...
    parser.add_argument('--consis_mode', type=str, default='lrud', help='lr, ud, lrud, left and right or up and down')
    ...
    parser.add_argument('--name', default='ETIS-Larib_consis', help='save to project/name') # public_consis_lrud_auglrud_st student teacher的输入交换
    # public_consis_100_coslr_0.00001initlr_postive_woconf_s2l_woflipaug: 100epoch, without conf consistency loss, consis loss weight small to large, without lrflip and udflip aug
    parser.add_argument('--exist-ok', action='store_true', help='existing project/name ok, do not increment')
    parser.add_argument('--quad', action='store_true', help='quad dataloader')
    parser.add_argument('--cos_lr', default=True, help='cosine LR scheduler')
