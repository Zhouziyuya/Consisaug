...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        if torch.is_tensor(self.alpha):
            a = self.alpha.clone()
            a = a.repeat(true.shape[0], 1) # 在第0维以true的长度复制
        else:
            a = self.alpha
        # print(a.shape)
        # print(true.shape)
        alpha_factor = true * a + (1 - true) * (1 - a)
...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.cp, self.cn = smooth_BCE(eps=h.get('label_smoothing', 0.0))  # positive, negative BCE targets (smooth targets) 默认没用到label smooth

        # Focal loss
        g = h['fl_gamma']  # focal loss gamma
        
        if g > 0:
            alpha = torch.tensor(h['fl_alpha'], dtype=torch.float32).to(device)
            BCEcls, BCEobj = FocalLoss(BCEcls, g, alpha), FocalLoss(BCEobj, g)
...
class ConsistencyLossLRUD: # 上下翻转的consistency loss
    def __init__(self, model):
        device = next(model.parameters()).device  # get model device
        self.kldivloss = nn.KLDivLoss(log_target=True, reduction='batchmean').to(device)
        self.mseloss = nn.MSELoss().to(device)
    ...
    def __call__(self, pred, pred_flip, targets):
        ...
        for i, pi in enumerate(pred):  # layer index, layer predictions
            ...
            if n:
                ...
                # consis_conf += (self.kldivloss(pconf.sigmoid(), pconf_consis.sigmoid()) + self.kldivloss(pconf_consis.sigmoid(), pconf.sigmoid()))/2
                consis_xy += (self.mseloss(py.sigmoid()*2-0.5, 1.5-py_consis.sigmoid()*2)+self.mseloss(px.sigmoid()*2-0.5, 1.5-px_consis.sigmoid()*2))/4
                consis_wh += self.mseloss(pwh.sigmoid(), pwh_consis.sigmoid())

        consis_loss = (consis_conf+consis_xy+consis_wh)/3

        return consis_loss, (consis_conf, consis_xy, consis_wh)


class ConsistencyLossNo: # 上下翻转的consistency loss
    def __init__(self, model):
        device = next(model.parameters()).device  # get model device
        self.kldivloss = nn.KLDivLoss(log_target=True, reduction='batchmean').to(device)
        self.mseloss = nn.MSELoss().to(device)
    ...
    def __call__(self, pred, pred_flip, targets):
        ...
        for i, pi in enumerate(pred):  # layer index, layer predictions
            ...
            if n:
                ...
                # consis_conf += (self.kldivloss(pconf.sigmoid(), pconf_consis.sigmoid()) + self.kldivloss(pconf_consis.sigmoid(), pconf.sigmoid()))/2
                consis_xy += (self.mseloss(py.sigmoid(), py_consis.sigmoid)+self.mseloss(px.sigmoid(), px_consis.sigmoid()))/2
                consis_wh += self.mseloss(pwh.sigmoid(), pwh_consis.sigmoid())
