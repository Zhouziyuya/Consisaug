...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_fliplrud = img_fliplrud.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        ...
        return torch.from_numpy(img), torch.from_numpy(img_fliplr), torch.from_numpy(img_flipud), torch.from_numpy(img_fliplrud), labels_out, self.im_files[index], shapes
    ...
    def collate_fn(batch):
        im, im_fliplr, im_flipud, im_fliplrud, label, path, shapes = zip(*batch)  # transposed
        for i, lb in enumerate(label):
            lb[:, 0] = i  # add target image index for build_targets()
        return torch.stack(im, 0), torch.stack(im_fliplr, 0), torch.stack(im_flipud, 0), torch.stack(im_fliplrud, 0), torch.cat(label, 0), path, shapes
