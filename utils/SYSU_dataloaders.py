...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            h,w,_ = im0.shape
            if h==576 and w==768:
                im1 = im1[90:490,270:700,:]

        if self.transforms:
            im = self.transforms(im1)  # transforms
        else:
            im = letterbox(im1, self.img_size, stride=self.stride, auto=self.auto)[0]  # padded resize
            im = im.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
            im = np.ascontiguousarray(im)  # contiguous

        if self.video_flag[0]:
            return path, im, im0, self.cap, s, self.frame # im0原图，im resize后的图
        else:
